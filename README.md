...
The **Javawookies Time Tracking System** is a desktop/laptop application targeted for small- to medium-sized businesses. The system is simply a *Login/Logout* application that allows two (2) types of users to log in. The users are classified into *Employee* and *Admin*, and both can sign in to their specific accounts and view the corresponding details. The user *Employee* has the ability to check its login/logout details or history and at the same time can delete time stamps, while the user *Admin* has the ability to add and delete time stamps as well as informations of recent and former employees. The system records the time of a specific user who logs in or out based on the local machine's timer.
